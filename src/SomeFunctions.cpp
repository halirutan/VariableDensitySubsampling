...
float SomeFunctions::randZeroOne()
{
	rand();
...
bool SomeFunctions::isOccurred(float probability)
{
	float randomNumber = randZeroOne();
	return (randomNumber < probability);
}

void SomeFunctions::insertParameters(long &nX,
									 long &nY,
									 double &M,
									 float &pF,
									 bool &pF_x,
									 ...
									 short int &subsampleType)
{
	if (flag_insertParam) {
		...
		do {
			...
			if (!((pF_x == 1) || (pF_x == 0)) || !(pF == 1 || pF == 0.5 || pF == 0.625 || pF == 0.75 || pF == 0.875)
				|| nX <= 0 || nX > 1024 || nY <= 0 || nY > 1024
...
void SomeFunctions::showParams(long nX,
							   long nY,
							   double M,
							   float pF,
							   bool pF_x,
